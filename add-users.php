...
$entryid=mt_rand(100000000, 999999999);
 ...
 $query=mysqli_query($con,"insert into tblusers(EntryID,UserName,UserAddress,MobileNumber,Email,DesignationName,IDProof) value('$entryid','$username','$uadd','$mobilenumber','$email','$cname','$idproof')");
                                        ...
                                        <div class="form-group"><label for="street" class=" form-control-label">User Address</label><textarea type="text" name="uadd" value="" id="uadd" class="form-control" required="true"></textarea></div>
                                            <div class="row form-group">
                                                <div class="col-12">
                                                    ...
                                                    <div class="col-12">
                                                    <div class="form-group"><label for="city" class=" form-control-label">Designation Name</label><select type="text" name="cname" id="cname" value="" class="form-control" required="true">
                                                    <option value="">Choose Designation</option>
                                <?php $query=mysqli_query($con,"select * from  tbldsign");
              while($row=mysqli_fetch_array($query))
              {
              ?>    
              <option value="<?php echo $row['DesignationName'];?>"><?php echo $row['DesignationName'];?></option>
                  <?php } ?>  
                                                    </select></div>
                                                    </div>
                                                    <div class="col-12">
                                                    <div class="form-group"><label for="city" class=" form-control-label">ID Proof</label><input type="file" name="idproof" id="idproof" value="" class="form-control" required="true"></div>
                                                    </div>
